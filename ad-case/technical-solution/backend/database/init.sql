...
-- HNSW 索引（用于向量检索）
-- 注意：这些索引在数据导入后创建会更高效
CREATE INDEX IF NOT EXISTS idx_ad_cases_combined_vector ON ad_cases
...
COMMENT ON TABLE ad_cases IS '广告案例主表';
COMMENT ON COLUMN ad_cases.combined_vector IS '标题+描述组合向量，用于语义检索（BGE-large-zh，1024维）';
COMMENT ON COLUMN ad_cases.combined_tsvector IS '标题+描述组合全文检索向量，用于关键词检索';
