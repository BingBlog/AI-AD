...
class AdquanAPIClient:
    ...
    def get_creative_list(self, page: int = 0, retry_count: int = 0) -> Dict[str, Any]:
        """
        获取创意案例列表
        ...
        Note:
            **API限制**：仅使用 page 参数时，最大可查询约593页。
            如需过滤数据，建议先爬取全部数据，然后在应用层进行过滤。
        """
        # 准备请求参数 - 仅包含 page 参数
        params = {
            'page': page,
        ...
        try:
            logger.info(f"请求案例列表 - page={page}")
    ...
    def get_creative_list_paginated(self, start_page: int = 0, max_pages: Optional[int] = 100) -> List[Dict[str, Any]]:
        """
        分页获取所有案例列表
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 测试分页获取（只获取2页作为测试）
        print(f"\n【测试3】分页获取案例（测试2页）...")
        all_items = client.get_creative_list_paginated(start_page=0, max_pages=2)
        print(f"✓ 分页获取完成")
