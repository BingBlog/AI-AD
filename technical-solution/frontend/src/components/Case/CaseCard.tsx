...
const CaseCard: React.FC<CaseCardProps> = ({ caseData, query }) => {
  ...
  const getImageUrl = (): string | undefined => {
    if (!caseData.main_image) {
      return undefined;
    }

    // 如果是相对路径（以 /static/ 开头），统一处理为本地图片
    ...
    if (caseData.main_image.startsWith("/static/")) {
      if (import.meta.env.DEV) {
        ...
        const backendUrl = import.meta.env.VITE_API_BASE_URL
          ...
          : "";
        return backendUrl
          ? `${backendUrl}${caseData.main_image}`
          : caseData.main_image;
      }
    }

    // 如果是完整URL（外部URL），直接返回（向后兼容）
    // 注意：后端应该已经优先返回本地图片路径，这里不应该经常走到
    return caseData.main_image;
  };
          ...
          <div className={styles.imageContainer}>
            <Image
              ...
              onError={(e) => {
                ...
                if (import.meta.env.DEV) {
                  console.error(
                    `图片加载失败: ${imageUrl} (案例ID: ${caseData.case_id})`,
                    {
                      error: e,
                      imageUrl,
                      caseId: caseData.case_id,
                      mainImage: caseData.main_image,
                    }
                  );
                }
              }}
        ...
        <div className={styles.meta}>
          <Space size="small" wrap>
            ...
            {caseData.agency_name && (
              <Tag color="geekblue" icon={<BuildOutlined />}>
                {caseData.agency_name}
              </Tag>
            )}
          </Space>
        ...
        {caseData.description && (
          <Tooltip
            ...
            <Paragraph className={styles.description} type="secondary">
              {query
                ? highlightText(getDescriptionText(), query)
                : getDescriptionText()}
            </Paragraph>
          </Tooltip>
        )}
